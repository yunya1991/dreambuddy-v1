...
export async function routeIntent(
  ...
  if (useLocalFirst && localResult.confidence >= 0.5) {
    return {
      ...localResult,
      skills: getSkillsForIntent(localResult.intent),
    } as RoutingResult;
  }
  ...
  try {
    ...
    if (response.success) {
      return {
        intent: response.intent?.primary_intent || 'unknown',
        ...
        action: INTENT_ACTION_MAP[response.intent?.primary_intent || 'unknown'],
