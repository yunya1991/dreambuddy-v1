...
`ops-ui` 当前不是主仓已存在服务，本文档描述的是目标形态。

| 项目 | 状态 | 说明 |
|---|---|---|
| `7-ARTIFACT-HUB-V2/src/ops-ui/` 目录 | 规划中 | 当前主仓尚无该目录 |
| `3457` 端口服务 | 规划中 | 仅作为未来本地开发默认端口预留，不代表当前主仓已可运行 |
| `GET /`、`GET /ui-map` | 规划中 | 当前主仓尚无 `ops-ui` Web 入口 |
| `GET /api/ops/*` | 规划中 | 当前主仓尚无对应子服务接口 |
...
- 研究中台以遗留 `/feed` 设计为基线；
- `ops-ui` 负责解释研究产物背后的 route / trace / audit。

### 6.2 与 `/chain` 的关系

- `/chain` 以遗留交易监控页为设计基线；
- `ops-ui` 负责从 route、trace、task / result、审计层解释其执行过程。

### 6.3 与市场化中台的关系

- 市场化中台负责内容路由、用户分层、推送分发、投放管理；
- `ops-ui` 提供治理证据链、风控边界和审计追踪。

### 6.4 与董事会总览台的关系

- 董事会总览台负责跨部门管理视角；
- `ops-ui` 负责提供治理底座和证据入口。

## 7. 当前限制

在真正落地前，`ops-ui` 仍有这些明确限制：
...
> 一个规划中的 `7-ARTIFACT-HUB-V2/src/ops-ui/` Node/TypeScript 子服务，用于承接治理控制台页面与治理代理接口，而不是当前主仓已存在的独立前端应用。
