...
`conflict_gate` 只能感知本地 git 状态，无法感知"对方准备做但尚未提交"的意图。
每次任务必须通过 PR 评论公开广播占用范围，弥补这一盲区。

### 强制执行顺序

1. 先运行 `dual-agent-conflict-gate`
2. 结果为 `SAFE` 或 `WARNING` → 先在协作主线 PR 发 `STARTED` 评论，再开始动手
3. 任务范围变化 → 发 `UPDATED` 评论
4. 门禁输出 `BLOCK` 或中途受阻 → 发 `BLOCKED` 评论
