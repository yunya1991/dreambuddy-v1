...
def generate_tasks(*, goal_id: str, task_prefix: str, workspace_path: str, plan_text: str):
    ...
    open_tasks = [t["task_id"] for t in tasks if t["status"] in {"planned", "open", "accepted", "rework", "blocked"}]
...
def apply_to_ledger(repo_root: Path, plan_out: dict):
    ...
    if not ledger_path.exists():
        raise SystemExit("ledger_tasks_index_not_found")
    ...
    open_ids = []
    for tid in (existing.get("open_tasks") or []):
        s = str(tid or "").strip()
        if s:
            open_ids.append(s)
    ...
    return {"written": str(ledger_path), "added": len(new_tasks)}
