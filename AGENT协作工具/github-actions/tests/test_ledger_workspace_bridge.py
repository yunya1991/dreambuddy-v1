...
class LedgerWorkspaceBridgeTests(unittest.TestCase):
                ...
                json.dumps(
                    {
                        ...
                        "tasks": [
                            ...
                            {"task_id": "task-ledger-x", "status": "planned", "workspace_path": "OTHER"},
                        ],
                    },
                    ensure_ascii=False,
                    indent=2,
    ...
    def test_fallback_to_status_derived_open_tasks_when_open_tasks_empty(self):
        with tempfile.TemporaryDirectory() as tmp:
            repo_root = Path(tmp)
            artifacts_root = repo_root / "dreambuddy" / "artifacts"
                ...
                json.dumps(
                    {
                        ...
                        "open_tasks": [],
                        "tasks": [
                            {"task_id": "task-ledger-1", "status": "planned", "workspace_path": "7-ARTIFACT-HUB-V2"},
                            {"task_id": "task-ledger-2", "status": "ledgered", "workspace_path": "7-ARTIFACT-HUB-V2"},
                        ],
                    },
                    ensure_ascii=False,
                    indent=2,
            ...
            snapshot = MODULE.build_snapshot(repo_root, artifacts_root, "7-ARTIFACT-HUB-V2", 200)
            self.assertEqual(snapshot["open_tasks_total"], 1)
            self.assertEqual(snapshot["items"][0]["ledger_task"]["task_id"], "task-ledger-1")
